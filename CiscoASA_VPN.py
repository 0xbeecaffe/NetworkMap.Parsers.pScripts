      ...
      for thisConnectionIndex in connectionIndexes:
        try:
          ...
          for tunnel_RemoteAddress in thisConnectionRemoteAddresses:
            ...
            thisConnectionISAKMP = Session.ExecCommand("show crypto isakmp sa | i {0}".format(tunnel_RemoteAddress))
            # extract the first ip address from result, that will be the local address
            tunnel_LocalAddress = GetIndexedIPAddressFromLine(thisConnectionISAKMP, 1)
            if tunnel_LocalAddress :
              ...
              nRegistry.RegisterTunnel(self.Router, instance, L3Discovery.NeighborProtocol.IPSEC, L3Discovery.LinkType.P2P, None, "Established", tunnel_LocalAddress.strip(), tunnel_RemoteAddress.strip(), None, None, s_localProxies, s_remoteProxies, cipherAlg, hashAlg, None) 
